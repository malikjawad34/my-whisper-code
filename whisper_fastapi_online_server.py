...
def is_english(text):
    try:
        return bool(text.strip()) and detect(text) == 'en'
...
async def websocket_endpoint(websocket: WebSocket):
    try:
        ...
        async def ffmpeg_stdout_reader():
            nonlocal pcm_buffer, last_speech_time, SilenceDetected
                    ...
                    if len(pcm_buffer) >= BYTES_PER_SEC:
                        ...
                        if trans:                           
                            if beg_trans is not None:
                                last_speech_time = time()  # Update only when English is detected
                            if last_speech_time is None:
                                last_speech_time = time()  # Start the timer on first transcription

                            print(f"English: {is_english(trans)}")
                            #if(is_english(trans)):
                            if trans != "Thank you.":
                        ...
                        if SilenceDetected:
                            print(f"Silence detected in ffmpeg_stdout_reader. Breaking out")
        ...
        async def monitor_silence():
            nonlocal last_speech_time, SilenceDetected
            while True:
                await asyncio.sleep(1)
                if last_speech_time and ((time() - last_speech_time) > 5):
                    print("No speech detected for x seconds. Preparing to close connection.")
                    print(f"Last Time    : {last_speech_time}")
        ...
        try:
            while True:
                ...
                if SilenceDetected:
                    print(f"Silence detected. Closing websocket")
            ...
            try:
                ...
                ffmpeg_process.wait(timeout=2)  # Allow it to exit gracefully
            except Exception as e:
